#include "mms_client_test_common.h"

#define IEC61850_MMS_CLIENT_DISABLE_MAIN
#include "../client/src/iec61850_mms_client.c"

static cJSON *on_request(char *method, cJSON *args, char **error);

static cJSON *test_invoke_api(const char *method, cJSON *args, char **error_out)
{
    char *error = NULL;
    cJSON *response = on_request((char *)method, args, &error);
    if (error_out) *error_out = error;
    return response;
}

static void run_write_load(int iterations, LoadStat *stat)
{
    stat->name = "write_items";
    stat->iterations = iterations;
    stat->success = 0;
    stat->failure = 0;

    test_set_connected(true);

    double start_ms = test_now_ms();
    for (int i = 0; i < iterations; i++) {
        char *error = NULL;
        cJSON *args = test_make_write_args("MockLD1/LLN0.Mod.ctlModel", "CF", "Int", i);
        cJSON *response = test_invoke_api("write_items", args, &error);
        cJSON_Delete(args);

        if (response != NULL && error == NULL && cJSON_IsObject(response)) {
            cJSON *status = cJSON_GetObjectItemCaseSensitive(response, "MockLD1/LLN0.Mod.ctlModel");
            if (cJSON_IsString(status)) {
                stat->success++;
            } else {
                stat->failure++;
            }
        } else {
            stat->failure++;
        }

        if (response) cJSON_Delete(response);
    }
    stat->elapsed_ms = test_now_ms() - start_ms;
}

int main(void)
{
    int write_iters = test_env_int("MMS_WRITE_ITERS", 20000);
    LoadStat stat;

    test_reset_state();
    run_write_load(write_iters, &stat);

    printf("C API write_items load test (mocked backend)\n");
    test_print_stat(&stat);

    return (stat.failure == 0) ? 0 : 1;
}
